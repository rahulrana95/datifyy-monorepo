...
export class DateCurationMapper {
  ...
  toCuratedDateResponse(date: DatifyyCuratedDates, currentUserId?: number): CuratedDateResponse {
    const now = new Date();
    ...
    return {
      ...
      suggestedConversationTopics: date.suggestedConversationTopics || undefined,
      user1ConfirmedAt: date.user1ConfirmedAt?.toISOString(),
      user2ConfirmedAt: date.user2ConfirmedAt?.toISOString(),
      cancelledByUserId: date.cancelledByUserId || undefined,
      ...
      reminderSent24h: date.reminderSent24h,
      reminderSent2h: date.reminderSent2h,
      followUpSent: date.followUpSent,
      compatibilityScore: date.compatibilityScore || undefined,
      matchReason: date.matchReason || undefined,
      algorithmConfidence: date.algorithmConfidence ? parseFloat(date.algorithmConfidence) : undefined,
      tokensCostUser1: date.tokensCostUser1,
      tokensCostUser2: date.tokensCostUser2,
      curatedBy: date.curatedBy,
      curatedAt: date.curatedAt.toISOString(),
      updatedBy: date.updatedBy || undefined,
      ...
      // Helper fields
      canCancel: currentUserId ? this.canCancelDate(date, currentUserId, hoursUntilDate) : false,
      canConfirm: currentUserId ? this.canConfirmDate(date, currentUserId) : false,
      requiresConfirmation: currentUserId ? this.requiresConfirmation(date, currentUserId) : false,
      hoursUntilDate: Math.max(0, Math.round(hoursUntilDate)),
      formattedDateTime: dateTime.toLocaleString(),
      isUpcoming: dateTime > now,
      canSubmitFeedback: currentUserId ? this.canSubmitFeedback(date, currentUserId) : false
    };
  }

  toDateFeedbackResponse(feedback: DatifyyCuratedDateFeedback, date?: DatifyyCuratedDates): DateFeedbackResponse {
    return {
      id: feedback.id,
      ...
      preferredNextDateActivity: feedback.preferredNextDateActivity || undefined,
  ...
  toUserTrustScoreResponse(trustScore: DatifyyUserTrustScores): UserTrustScoreResponse {
    return {
      id: trustScore.id,
      userId: trustScore.userId,
      overallScore: trustScore.overallScore,
      attendanceScore: trustScore.attendanceScore,
      punctualityScore: trustScore.punctualityScore,
      feedbackScore: trustScore.feedbackScore,
      profileCompletenessScore: trustScore.profileCompletenessScore,
      totalDatesAttended: trustScore.totalDatesAttended,
      totalDatesCancelled: trustScore.totalDatesCancelled,
      totalDatesNoShow: trustScore.totalDatesNoShow,
      lastMinuteCancellations: trustScore.lastMinuteCancellations,
      averageRating: parseFloat(trustScore.averageRating),
      consecutiveCancellations: trustScore.consecutiveCancellations,
      lastCancellationDate: trustScore.lastCancellationDate?.toISOString(),
      warningLevel: trustScore.warningLevel,
      isOnProbation: trustScore.isOnProbation,
      probationUntil: trustScore.probationUntil?.toISOString(),
      secondDateRate: parseFloat(trustScore.secondDateRate),
      positiveFeedbackCount: trustScore.positiveFeedbackCount,
      complimentsReceived: trustScore.complimentsReceived,
      canBookDates: trustScore.canBookDates,
      maxDatesPerWeek: trustScore.maxDatesPerWeek,
      requiresAdminApproval: trustScore.requiresAdminApproval,
      lastCalculatedAt: trustScore.lastCalculatedAt.toISOString(),
      calculationReason: trustScore.calculationReason || undefined,

      scoreBreakdown: {
        attendance: {
          score: trustScore.attendanceScore,
          description: this.getAttendanceDescription(trustScore.attendanceScore),
          impactFactors: this.getAttendanceImpactFactors(trustScore)
        },
        punctuality: {
          score: trustScore.punctualityScore,
          description: this.getPunctualityDescription(trustScore.punctualityScore),
          impactFactors: this.getPunctualityImpactFactors(trustScore)
        },
        feedback: {
          score: trustScore.feedbackScore,
          description: this.getFeedbackDescription(trustScore.feedbackScore),
          impactFactors: this.getFeedbackImpactFactors(trustScore)
        },
        profileCompleteness: {
          score: trustScore.profileCompletenessScore,
          description: this.getProfileCompletenessDescription(trustScore.profileCompletenessScore),
      ...
      comparisonToAverage: {
        ...
        categoryRanking: this.getCategoryRanking(trustScore.overallScore)
      }
    };
  }

  toDateSeriesResponse(series: DatifyyDateSeries, currentUserId?: number): DateSeriesResponse {
    return {
      id: series.id,
      user1Id: series.user1Id,
      user2Id: series.user2Id,
      seriesStatus: series.seriesStatus,
      totalDatesInSeries: series.totalDatesInSeries,
      lastDateAt: series.lastDateAt?.toISOString(),
      nextSuggestedDate: series.nextSuggestedDate?.toISOString(),
      relationshipStage: series.relationshipStage,
      ...
      endedByUserId: series.endedByUserId || undefined,
      ...
      datesInSeries: series.curatedDates?.map(date => this.toCuratedDateResponse(date, currentUserId)) || [],

      progressionSummary: {
        currentStage: series.relationshipStage,
      ...
      seriesStats: {
        ...
        recommendNextDate: series.seriesStatus === 'active',
        suggestedNextActivity: this.getSuggestedNextActivity(series)
      }
    };
  }

  toPotentialMatchResponse(user: any, searcherUserId: number): PotentialMatchResponse {
    return {
      userId: user.id,
      userInfo: {
        id: user.id,
      ...
      matchReasons: ['Similar interests', 'Compatible age range', 'Shared values'],
  ...
  // Helper methods
  private canCancelDate(date: DatifyyCuratedDates, userId: number, hoursUntilDate: number): boolean {
    if (date.status === CuratedDateStatus.CANCELLED || date.status === CuratedDateStatus.COMPLETED) {
      return false;
    }
  ...
  private requiresConfirmation(date: DatifyyCuratedDates, userId: number): boolean {
    return this.canConfirmDate(date, userId) && date.status === CuratedDateStatus.PENDING;
  }

  private canSubmitFeedback(date: DatifyyCuratedDates, userId: number): boolean {
    return date.status === CuratedDateStatus.COMPLETED && 
           (date.user1Id === userId || date.user2Id === userId);
  }

  private canEditFeedback(feedback: DatifyyCuratedDateFeedback): boolean {
    const hoursAfterSubmission = (new Date().getTime() - new Date(feedback.submittedAt).getTime()) / (1000 * 60 * 60);
  ...
  private getAttendanceImpactFactors(trustScore: DatifyyUserTrustScores): string[] {
    const factors = [];
    if (trustScore.totalDatesAttended > 5) factors.push('Multiple completed dates');
    if (trustScore.totalDatesCancelled > 2) factors.push('Some cancelled dates');
    if (trustScore.totalDatesNoShow > 0) factors.push('No-show incidents');
    return factors;
  }

  private getPunctualityImpactFactors(trustScore: DatifyyUserTrustScores): string[] {
    return ['Based on date partner feedback'];
  }

  private getFeedbackImpactFactors(trustScore: DatifyyUserTrustScores): string[] {
    const factors = [];
    if (trustScore.positiveFeedbackCount > 3) factors.push('Multiple positive reviews');
    if (parseFloat(trustScore.averageRating) > 4) factors.push('High average rating');
    return factors;
  }

  private getProfileCompletenessImpactFactors(trustScore: DatifyyUserTrustScores): string[] {
    return ['Profile information completeness', 'Verification status'];
  }

  private getTrustScoreRecommendations(trustScore: DatifyyUserTrustScores): string[] {
    const recommendations = [];
    
    if (trustScore.profileCompletenessScore < 80) {
      recommendations.push('Complete your profile information');
    }
    
    if (trustScore.consecutiveCancellations > 1) {
      recommendations.push('Avoid cancelling dates to improve reliability');
    }
    
    if (parseFloat(trustScore.averageRating) < 4) {
  ...
  private getNextReviewDate(trustScore: DatifyyUserTrustScores): string {
    const nextReview = new Date(trustScore.lastCalculatedAt);
  ...
  private calculateStageProgress(series: DatifyyDateSeries): number {
    const stageOrder = ['getting_to_know', 'building_connection', 'strong_interest', 'exclusive_dating', 'relationship'];
    const currentIndex = stageOrder.indexOf(series.relationshipStage);
  ...
  private getSuggestedNextActivity(series: DatifyyDateSeries): string | undefined {
    const activities = {
    ...
    return activities[series.relationshipStage];
  }
}